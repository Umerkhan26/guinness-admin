...
import { FiArrowLeft, FiBriefcase, FiInfo, FiCheckCircle, FiXCircle, FiMail, FiPhone, FiMapPin, FiAward } from 'react-icons/fi';
...
const Badge = styled.span<{ $variant?: 'primary' | 'success' | 'warning' | 'muted' | 'danger' }>`
...
const BusinessDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchBusinessDetails = async () => {
      ...
      try {
        const response = await getUserById(businessId);
        if (response.success) {
        ...
        const errorMessage = 'Unexpected error while fetching business details.';
    ...
    return (
      <PageContainer>
        <BreadcrumbContainer>
          ...
          <BreadcrumbCurrent>Business Details</BreadcrumbCurrent>
        </BreadcrumbContainer>
        <LoadingContainer>Loading business details...</LoadingContainer>
      </PageContainer>
    );
  }
                  ...
                  <TableRow>
                    ...
                    <TableValue>
                      <Badge $variant={userDetails.businessInfo.approvedByAdmin ? 'success' : 'warning'}>
                        {userDetails.businessInfo.approvedByAdmin ? (
                          <>
                            <FiCheckCircle style={{ marginRight: '0.25rem', verticalAlign: 'middle' }} />
                            Approved
                          </>
                        ) : (
                          <>
                            <FiXCircle style={{ marginRight: '0.25rem', verticalAlign: 'middle' }} />
                            Pending
                          </>
                        )}
                      </Badge>
